...
public class TestASRAInstruction {
  ...
  public void testSimpleASRAWithExtraSpace() {
    try {
      ...
      Assert.assertEquals("Must be an ASRA instruction", "ASRA", aslInstruction.getInstruction());
    } catch (Throwable _e) {
   ...
   */
  @Test
  public void testASRAWithDuplicateLabel() {
    try {
      StringConcatenation _builder = new StringConcatenation();
      _builder.append("; -----------------------------------------");
      _builder.newLine();
  ...
  public void testASRAAssembly() {
    try {
      ...
      _builder.append("ORG    \t\t\t$8000");
      _builder.newLine();
      _builder.append("Start      \tASRA\t\t  \t\t    ; 47   ASRA");
      _builder.newLine();
      final Model result = this.parseHelper.parse(_builder);
      Assert.assertNotNull(result);
      this._validationTestHelper.assertNoErrors(result);
      ...
      final AssembledASRAInstruction line = ((AssembledASRAInstruction) _assembledLine);
      Assert.assertEquals("Check opcode length", 1, line.getOpcode().length);
      Assert.assertEquals("Check opcode", 0x47, line.getOpcode()[0]);
      Assert.assertEquals("Check operand length", 0, line.getOperand().length);
      Assert.assertEquals("Check label", "Start", line.getLabel());
      Assert.assertEquals("Check comment", "; 47   ASRA", line.getComment());
    } catch (Throwable _e) {
      throw Exceptions.sneakyThrow(_e);
    }
  }

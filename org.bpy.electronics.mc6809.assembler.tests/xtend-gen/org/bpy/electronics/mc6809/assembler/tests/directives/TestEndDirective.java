...
public class TestEndDirective {
  ...
  public void testEndUnexpectedLabel() {
    try {
      ...
      final Model result = this.parseHelper.parse(_builder);
      Assert.assertNotNull(result);
      this._validationTestHelper.assertError(result, AssemblerPackage.eINSTANCE.getDirectiveLine(), DirectiveValidator.UNEXPECTED_LABEL, "No label may be set for END directive");
    } catch (Throwable _e) {
      throw Exceptions.sneakyThrow(_e);
    }
  }

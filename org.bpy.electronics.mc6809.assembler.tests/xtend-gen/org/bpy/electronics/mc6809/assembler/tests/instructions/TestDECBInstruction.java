...
public class TestDECBInstruction {
  ...
  public void testSimpleDECBWithExtraSpace() {
    try {
      ...
      Assert.assertEquals("Must be an DECB instruction", "DECB", aslInstruction.getInstruction());
    } catch (Throwable _e) {
  ...
  public void testDECBAssembly() {
    try {
      ...
      _builder.append("ORG    \t\t\t$8000");
      _builder.newLine();
      _builder.append("Start      \tDECB\t\t  \t\t    ; 5A   DECB");
      _builder.newLine();
      final Model result = this.parseHelper.parse(_builder);
      Assert.assertNotNull(result);
      this._validationTestHelper.assertNoErrors(result);
      ...
      final AssembledDECBInstruction line = ((AssembledDECBInstruction) _assembledLine);

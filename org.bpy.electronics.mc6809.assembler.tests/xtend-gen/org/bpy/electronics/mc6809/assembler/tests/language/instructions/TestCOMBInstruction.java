...
public class TestCOMBInstruction {
  @Inject
  ...
  public void testSimpleCOMBWithExtraSpace() {
    try {
      ...
      Assert.assertEquals("Must be an COMB instruction", "COMB", aslInstruction.getInstruction());
    } catch (Throwable _e) {
  ...
  /**
   * Check COMB instruction with duplicate label
   */
  @Test
  public void testCOMBWithDuplicateLabel() {
    try {
      StringConcatenation _builder = new StringConcatenation();
      _builder.append("; -----------------------------------------");
      _builder.newLine();
  ...
  /**
   * Check COMB assembly instruction
   */
  @Test
  public void testCOMBAssembly() {
    try {
      StringConcatenation _builder = new StringConcatenation();
      _builder.append("; -----------------------------------------");
      _builder.newLine();
      ...
      _builder.append("ORG    \t\t\t$8000");
      _builder.newLine();
      _builder.append("Start      \tCOMB\t\t  \t\t    ; 53   COMB");
      _builder.newLine();
      final Model result = this.parseHelper.parse(_builder);
      Assert.assertNotNull(result);
      this._validationTestHelper.assertNoErrors(result);
      ...
      final AssembledCOMBInstruction line = ((AssembledCOMBInstruction) _assembledLine);
      Assert.assertEquals("Check opcode length", 1, line.getOpcode().length);
      Assert.assertEquals("Check opcode", 0x53, line.getOpcode()[0]);
      Assert.assertEquals("Check operand length", 0, line.getOperand().length);
      Assert.assertEquals("Check label", "Start", line.getLabel());
      Assert.assertEquals("Check comment", "; 53   COMB", line.getComment());
    } catch (Throwable _e) {
      throw Exceptions.sneakyThrow(_e);
    }
  }

...
public class TestPULUInstruction {
   ...
   */
  @Test
  public void testPULUImemdiatAddressingMode1() {
    try {
      ...
      Assert.assertTrue("Must be an PULU directive line", (_instruction instanceof PuluInstruction));
      EObject _instruction_1 = instructionLine.getInstruction();
      final PuluInstruction puluInstruction = ((PuluInstruction) _instruction_1);
      ImmediatOperand _operand = puluInstruction.getOperand();
  ...
  public void testPULUImemdiatAddressingMode3() {
    try {
      ...
      _builder.newLine();
      _builder.append("\t       ");
      _builder.append("PULU\t\t  \tPC,U,Y,X,DP,B,A,CC");
      _builder.newLine();
      final Model result = this.parseHelper.parse(_builder);
      Assert.assertNotNull(result);
      this._validationTestHelper.assertNoErrors(result);
      ...
      EObject _instruction_1 = instructionLine.getInstruction();
      final PuluInstruction puluInstruction = ((PuluInstruction) _instruction_1);
      Assert.assertEquals("Must be an PULU instruction", puluInstruction.getInstruction(), "PULU");
    } catch (Throwable _e) {
      throw Exceptions.sneakyThrow(_e);
    }
  }

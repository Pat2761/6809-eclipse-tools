...
public class TestDAAInstruction {
  ...
  public void testSimpleDAAWithExtraSpace() {
    try {
      ...
      Assert.assertTrue("Must be an DAA Accumulator line", (_instruction instanceof DaaInstruction));
      EObject _instruction_1 = instructionLine.getInstruction();
      final DaaInstruction aslInstruction = ((DaaInstruction) _instruction_1);
      Assert.assertEquals("Must be an DAA instruction", "DAA", aslInstruction.getInstruction());
    } catch (Throwable _e) {
  ...
  public void testSimpleDAAWithExtraSpaceWithComment() {
    try {
      ...
      _builder.newLine();
      _builder.append("\t       ");
      _builder.append("DAA  \t\t\t; It is a comment ");
      _builder.newLine();
      final Model result = this.parseHelper.parse(_builder);
      Assert.assertNotNull(result);
      this._validationTestHelper.assertNoErrors(result);

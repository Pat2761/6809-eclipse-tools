...
public class TestCLRAInstruction {
  ...
  public void testSimpleCLRAWithExtraSpace() {
    try {
      ...
      Assert.assertEquals("Must be an CLRA instruction", "CLRA", aslInstruction.getInstruction());
    } catch (Throwable _e) {
   ...
   */
  @Test
  public void testCLRAWithDuplicateLabel() {
    try {
      StringConcatenation _builder = new StringConcatenation();
      _builder.append("; -----------------------------------------");
      _builder.newLine();
  ...
  public void testCLRAAssembly() {
    try {
      ...
      _builder.append("ORG    \t\t\t$8000");
      _builder.newLine();
      _builder.append("Start      \tCLRA\t\t  \t\t    ; 4F   CLRA");
      _builder.newLine();
      final Model result = this.parseHelper.parse(_builder);
      Assert.assertNotNull(result);
      this._validationTestHelper.assertNoErrors(result);
      ...
      final AssembledCLRAInstruction line = ((AssembledCLRAInstruction) _assembledLine);
      Assert.assertEquals("Check opcode length", 1, line.getOpcode().length);
      Assert.assertEquals("Check opcode", 0x4F, line.getOpcode()[0]);
      Assert.assertEquals("Check operand length", 0, line.getOperand().length);
      Assert.assertEquals("Check label", "Start", line.getLabel());
      Assert.assertEquals("Check comment", "; 4F   CLRA", line.getComment());
    } catch (Throwable _e) {
      throw Exceptions.sneakyThrow(_e);
    }
  }

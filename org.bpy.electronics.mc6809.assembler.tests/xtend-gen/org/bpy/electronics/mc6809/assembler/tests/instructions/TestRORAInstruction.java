...
public class TestRORAInstruction {
  @Inject
  ...
  public void testSimpleRORAWithExtraSpace() {
    try {
      ...
      Assert.assertTrue("Must be an Lsr Accumulateur line", (_instruction instanceof RorInstruction));
   ...
   */
  @Test
  public void testRORAWithDuplicateLabel() {
    try {
      StringConcatenation _builder = new StringConcatenation();
      _builder.append("; -----------------------------------------");
      _builder.newLine();
  ...
  public void testRORAAssembly() {
    try {
      ...
      _builder.append("ORG    \t\t\t$8000");
      _builder.newLine();
      _builder.append("Start      \tRORA\t\t  \t\t    ; 46   RORA");
      _builder.newLine();
      final Model result = this.parseHelper.parse(_builder);
      Assert.assertNotNull(result);
      this._validationTestHelper.assertNoErrors(result);
      ...
      final AssembledRORAInstruction line = ((AssembledRORAInstruction) _assembledLine);
      Assert.assertEquals("Check opcode length", 1, line.getOpcode().length);
      Assert.assertEquals("Check opcode", 0x46, line.getOpcode()[0]);
      Assert.assertEquals("Check operand length", 0, line.getOperand().length);
      Assert.assertEquals("Check label", "Start", line.getLabel());
      Assert.assertEquals("Check comment", "; 46   RORA", line.getComment());
    } catch (Throwable _e) {
      throw Exceptions.sneakyThrow(_e);
    }
  }

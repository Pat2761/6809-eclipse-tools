...
public class TestASRBInstruction {
  ...
  public void testSimpleASRBWithExtraSpace() {
    try {
      ...
      Assert.assertTrue("Must be an ASR Accumulator line", (_instruction instanceof AsrInstruction));
      EObject _instruction_1 = instructionLine.getInstruction();
      final AsrInstruction aslInstruction = ((AsrInstruction) _instruction_1);
      Assert.assertEquals("Must be an ASRB instruction", "ASRB", aslInstruction.getInstruction());
    } catch (Throwable _e) {
   ...
   */
  @Test
  public void testASRBWithDuplicateLabel() {
    try {
      StringConcatenation _builder = new StringConcatenation();
      _builder.append("; -----------------------------------------");
      _builder.newLine();
  ...
  /**
   * Check ASRB assembly instruction
   */
  @Test
  public void testASRBAssembly() {
    try {
      StringConcatenation _builder = new StringConcatenation();
      _builder.append("; -----------------------------------------");
      _builder.newLine();
      ...
      _builder.append("ORG    \t\t\t$8000");
      _builder.newLine();
      _builder.append("Start      \tASRB\t\t  \t\t    ; 57   ASRB");
      _builder.newLine();
      final Model result = this.parseHelper.parse(_builder);
      Assert.assertNotNull(result);
      this._validationTestHelper.assertNoErrors(result);
      ...
      final AssembledASRBInstruction line = ((AssembledASRBInstruction) _assembledLine);
      Assert.assertEquals("Check opcode length", 1, line.getOpcode().length);
      Assert.assertEquals("Check opcode", 0x57, line.getOpcode()[0]);
      Assert.assertEquals("Check operand length", 0, line.getOperand().length);
      Assert.assertEquals("Check label", "Start", line.getLabel());
      Assert.assertEquals("Check comment", "; 57   ASRB", line.getComment());
    } catch (Throwable _e) {
      throw Exceptions.sneakyThrow(_e);
    }
  }

...
public class TestCOMAInstruction {
  @Inject
  ...
  public void testSimpleCOMAWithExtraSpace() {
    try {
      ...
      Assert.assertTrue("Must be an COM Accumulator line", (_instruction instanceof ComInstruction));
      EObject _instruction_1 = instructionLine.getInstruction();
      final ComInstruction aslInstruction = ((ComInstruction) _instruction_1);
      Assert.assertEquals("Must be an COMA instruction", "COMA", aslInstruction.getInstruction());
    } catch (Throwable _e) {
  ...
  /**
   * Check COMA instruction with duplicate label
   */
  @Test
  public void testCOMAWithDuplicateLabel() {
    try {
      StringConcatenation _builder = new StringConcatenation();
      _builder.append("; -----------------------------------------");
      _builder.newLine();
  ...
  /**
   * Check COMA assembly instruction
   */
  @Test
  public void testCOMAAssembly() {
    try {
      StringConcatenation _builder = new StringConcatenation();
      _builder.append("; -----------------------------------------");
      _builder.newLine();
      ...
      _builder.append("ORG    \t\t\t$8000");
      _builder.newLine();
      _builder.append("Start      \tCOMA\t\t  \t\t    ; 43   COMA");
      _builder.newLine();
      final Model result = this.parseHelper.parse(_builder);
      Assert.assertNotNull(result);
      this._validationTestHelper.assertNoErrors(result);
      ...
      final AssembledCOMAInstruction line = ((AssembledCOMAInstruction) _assembledLine);
      Assert.assertEquals("Check opcode length", 1, line.getOpcode().length);
      Assert.assertEquals("Check opcode", 0x43, line.getOpcode()[0]);
      Assert.assertEquals("Check operand length", 0, line.getOperand().length);
      Assert.assertEquals("Check label", "Start", line.getLabel());
      Assert.assertEquals("Check comment", "; 43   COMA", line.getComment());
    } catch (Throwable _e) {
      throw Exceptions.sneakyThrow(_e);
    }
  }

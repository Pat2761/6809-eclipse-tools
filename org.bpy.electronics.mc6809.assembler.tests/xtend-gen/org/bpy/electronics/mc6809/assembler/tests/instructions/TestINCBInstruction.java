...
public class TestINCBInstruction {
  ...
  public void testSimpleINCBWithExtraSpace() {
    try {
      ...
      Assert.assertTrue("Must be an INC Accumulator line", (_instruction instanceof IncInstruction));
  ...
  public void testINCBAssembly() {
    try {
      ...
      _builder.append("ORG    \t\t\t$8000");
      _builder.newLine();
      _builder.append("Start      \tINCB\t\t  \t\t    ; 4C   INCB");
      _builder.newLine();
      final Model result = this.parseHelper.parse(_builder);
      Assert.assertNotNull(result);
      this._validationTestHelper.assertNoErrors(result);
      ...
      final AssembledINCBInstruction line = ((AssembledINCBInstruction) _assembledLine);
      ...
      Assert.assertEquals("Check comment", "; 4C   INCB", line.getComment());
    } catch (Throwable _e) {
      throw Exceptions.sneakyThrow(_e);
    }
  }

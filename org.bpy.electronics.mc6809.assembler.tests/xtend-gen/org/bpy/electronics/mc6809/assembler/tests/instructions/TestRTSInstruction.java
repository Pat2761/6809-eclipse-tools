...
public class TestRTSInstruction {
  ...
  public void testSimpleRTSWithExtraSpace() {
    try {
      ...
      Assert.assertTrue("Must be an Lsr Accumulateur line", (_instruction instanceof RtsInstruction));
      EObject _instruction_1 = instructionLine.getInstruction();
      final RtsInstruction RtsInstruction = ((org.bpy.electronics.mc6809.assembler.assembler.RtsInstruction) _instruction_1);
  ...
  public void testSimpleRTSWithExtraSpaceWithComment() {
    try {
      ...
      _builder.newLine();
      _builder.append("\t       ");
      _builder.append("RTS  \t\t\t; It is a comment ");
      _builder.newLine();
      final Model result = this.parseHelper.parse(_builder);
      Assert.assertNotNull(result);
      this._validationTestHelper.assertNoErrors(result);
  ...
  public void testRTSAssembly() {
    try {
      ...
      _builder.append("ORG    \t\t\t$8000");
      _builder.newLine();
      _builder.append("Start      \tRTS\t\t  \t\t    ; 39   RTS");
      _builder.newLine();
      final Model result = this.parseHelper.parse(_builder);
      Assert.assertNotNull(result);
      this._validationTestHelper.assertNoErrors(result);
      ...
      final AssembledRTSInstruction line = ((AssembledRTSInstruction) _assembledLine);

...
class TestCOMAInstruction {
	...
	def void testSimpleCOMAWithExtraSpace() {
		...
		Assert.assertEquals("Must be an COMA instruction", "COMA", aslInstruction.instruction)
	}
	 ...
	 */
	@Test 
	def void testCOMAWithDuplicateLabel() {
		val result = parseHelper.parse('''
		; -----------------------------------------
				   	ORG    			$8000
		Const	   	EQU          	5
	...
	def void testCOMAAssembly() {
		...
		val line = engine.getAssembledLine(2) as AssembledCOMAInstruction
		Assert.assertEquals("Check opcode length", 1, line.opcode.length)
		Assert.assertEquals("Check opcode", 0x43, line.opcode.get(0))
		Assert.assertEquals("Check operand length", 0, line.operand.length)
		Assert.assertEquals("Check label", "Start" , line.label)
		Assert.assertEquals("Check comment", "; 43   COMA" , line.comment)
	}
}
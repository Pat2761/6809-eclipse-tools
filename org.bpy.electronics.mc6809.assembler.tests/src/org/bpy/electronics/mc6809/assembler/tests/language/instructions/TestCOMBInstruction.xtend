...
class TestCOMBInstruction {
	...
	def void testSimpleCOMBWithExtraSpace() {
		...
		Assert.assertTrue("Must be an COM Accumulator line", instructionLine.instruction instanceof ComInstruction)
		val aslInstruction = instructionLine.instruction as ComInstruction
		Assert.assertEquals("Must be an COMB instruction", "COMB", aslInstruction.instruction)
	}
	 ...
	 */
	@Test 
	def void testCOMBWithDuplicateLabel() {
		val result = parseHelper.parse('''
		; -----------------------------------------
				   	ORG    			$8000
		Const	   	EQU          	5
	...
	def void testCOMBAssembly() {
		...
		val line = engine.getAssembledLine(2) as AssembledCOMBInstruction
		Assert.assertEquals("Check opcode length", 1, line.opcode.length)
		Assert.assertEquals("Check opcode", 0x53, line.opcode.get(0))
		Assert.assertEquals("Check operand length", 0, line.operand.length)
		Assert.assertEquals("Check label", "Start" , line.label)
		Assert.assertEquals("Check comment", "; 53   COMB" , line.comment)
	}
}
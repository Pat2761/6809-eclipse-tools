...
public class TestASLAInstruction {
	...
	public void testSimpleASLAWithExtraSpace() {
		...
		try {
		   ...
		   SourceLine line = result.getSourceLines().get(2);
			...
			Assert.assertTrue("Must be an ASLA directive line", instructionLine.getInstruction() instanceof AslInstruction);
		} catch (Exception e) {
	...
	public void testSimpleASLAWithExtraSpaceWithComment() {
		StringBuilder strBuilder = new StringBuilder();
		strBuilder.append("; -----------------------------------------\n");
		strBuilder.append("	       ASLA  			; It is a comment\n");
		strBuilder.append("	       ASLA\n"); 
		Model result;
		try {
			result = parseHelper.parse(strBuilder.toString());
	...
	/**
	 * Check Assembled ASLA
	 */
	@Test 
	public void testAssembledASLA() {
		StringBuilder strBuilder = new StringBuilder();
		strBuilder.append("; -----------------------------------------\n");
		strBuilder.append("	      	ORG    $8000\n");
		strBuilder.append("LabelAsla	ASLA					; Asla comment\n");
		try {
			Model result = parseHelper.parse(strBuilder.toString());
			...
			AssembledASLAInstruction line = (AssembledASLAInstruction)engine.getAssembledLine(2);
			Assert.assertEquals("Check label", "LabelAsla" , line.getLabel());
			Assert.assertEquals("Check comment", "; Asla comment" , line.getComment());
			...
			Assert.assertEquals("Check Opcode code",0x48,code[0]);

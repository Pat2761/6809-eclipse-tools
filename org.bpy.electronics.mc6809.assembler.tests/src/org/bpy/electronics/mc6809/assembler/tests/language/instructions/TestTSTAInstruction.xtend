...
class TestTSTAInstruction {
	...
	def void testSimpleTSTAWithExtraSpace() {
		...
		Assert.assertTrue("Must be an Lsr Accumulateur line", instructionLine.instruction instanceof TstInstruction)
	 ...
	 */
	@Test 
	def void testTSTAWithDuplicateLabel() {
		val result = parseHelper.parse('''
		; -----------------------------------------
				   	ORG    			$8000
		Const	   	EQU          	5
	...
	def void testTSTAAssembly() {
		...
		val line = engine.getAssembledLine(2) as AssembledTSTAInstruction
		Assert.assertEquals("Check opcode length", 1, line.opcode.length)
		Assert.assertEquals("Check opcode", 0x4D, line.opcode.get(0))
		Assert.assertEquals("Check operand length", 0, line.operand.length)
		Assert.assertEquals("Check label", "Start" , line.label)
		Assert.assertEquals("Check comment", "; 4D   TSTA" , line.comment)
	}
}
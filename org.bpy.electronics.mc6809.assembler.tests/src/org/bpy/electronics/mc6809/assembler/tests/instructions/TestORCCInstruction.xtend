...
class TestORCCInstruction {
	 ...
	 */
	@Test 
	def void testOCCImemdiatAddressingMode() {
		...
		Assert.assertTrue("Must be an ORCC directive line", instructionLine.instruction instanceof OrccInstruction)

		val orccInstruction = instructionLine.instruction as OrccInstruction
		assertEquals("Must be an ORA instruction", orccInstruction.instruction,"ORCC")

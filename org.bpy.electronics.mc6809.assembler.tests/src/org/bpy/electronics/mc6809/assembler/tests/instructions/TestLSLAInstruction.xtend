...
class TestLSLAInstruction {
	 ...
	 */
	@Test 
	def void testLSLAWithDuplicateLabel() {
		val result = parseHelper.parse('''
		; -----------------------------------------
				   	ORG    			$8000
		Const	   	EQU          	5
	...
	def void testLSLAAssembly() {
		...
		val line = engine.getAssembledLine(2) as AssembledLSLAInstruction
		...
		Assert.assertEquals("Check comment", "; 48   LSLA" , line.comment)
	}
}
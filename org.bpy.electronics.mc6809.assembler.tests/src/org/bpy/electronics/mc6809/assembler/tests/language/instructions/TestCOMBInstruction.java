...
public class TestCOMBInstruction {
	...
	public void testSimpleCOMBWithExtraSpace() {
		...
		try {
		   ...
		   SourceLine line = result.getSourceLines().get(2);
			...
			Assert.assertTrue("Must be an COMB directive line", instructionLine.getInstruction() instanceof ComInstruction);
		} catch (Exception e) {
	...
	public void testSimpleCOMBWithExtraSpaceWithComment() {
		StringBuilder strBuilder = new StringBuilder();
		strBuilder.append("; -----------------------------------------\n");
		strBuilder.append("	       COMB  			; It is a comment\n");
		strBuilder.append("	       COMB\n"); 
		Model result;
		try {
			result = parseHelper.parse(strBuilder.toString());
	...
	/**
	 * Check Assembled COMB
	 */
	@Test 
	public void testAssembledCOMB() {
		StringBuilder strBuilder = new StringBuilder();
		strBuilder.append("; -----------------------------------------\n");
		strBuilder.append("	      	ORG    $8000\n");
		strBuilder.append("LabelComb	COMB					; Comb comment\n");
		try {
			Model result = parseHelper.parse(strBuilder.toString());
			...
			AssembledCOMBInstruction line = (AssembledCOMBInstruction)engine.getAssembledLine(2);
			Assert.assertEquals("Check label", "LabelComb" , line.getLabel());
			Assert.assertEquals("Check comment", "; Comb comment" , line.getComment());
			...
			Assert.assertEquals("Check Opcode code",0x53,code[0]);

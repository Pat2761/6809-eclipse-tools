...
class TestNEGBInstruction {
	 ...
	 */
	@Test 
	def void testNEGBWithDuplicateLabel() {
		val result = parseHelper.parse('''
		; -----------------------------------------
				   	ORG    			$8000
		Const	   	EQU          	5
	...
	def void testNEGBAssembly() {
		...
		val line = engine.getAssembledLine(2) as AssembledNEGBInstruction
		Assert.assertEquals("Check opcode length", 1, line.opcode.length)
		Assert.assertEquals("Check opcode", 0x50, line.opcode.get(0))
		Assert.assertEquals("Check operand length", 0, line.operand.length)
		Assert.assertEquals("Check label", "Start" , line.label)
		Assert.assertEquals("Check comment", "; 50   NEGB" , line.comment)
	}
}
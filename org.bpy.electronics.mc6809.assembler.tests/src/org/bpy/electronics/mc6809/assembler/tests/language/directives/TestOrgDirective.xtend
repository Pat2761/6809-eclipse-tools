...
//		Assert.assertEquals("Operand must be equals to $4000", 0x4000, ExpressionParser.parse(orgDirective))		
	...
	def void testBszWithErrorExpression() {
		...
		; -----------------------------------------
			           	ORG    	$2000  		 	; With value
		TOTO	       	ORG    	10*Deux 		; Toto vaudra $2000
												; Et en mémoire entre $2000 et $2010, il y aura des 0
		''')
		Assert.assertNotNull(result)
		result.assertError(AssemblerPackage.eINSTANCE.orgDirective, ExpressionParser::EXPRESSION_ERROR, "Can't find Deux definition")
	}
	...
	def void testBszWithBinaryErrorSyntax() {
		...
		; -----------------------------------------
			           	ORG    	$2000  		 	; With value
		TOTO	       	ORG    	%01111211 		; Toto vaudra $2000
												; Et en mémoire entre $2000 et $2010, il y aura des 0
		''')
		Assert.assertNotNull(result)

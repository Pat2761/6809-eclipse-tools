...
class TestASRBInstruction {
	...
	def void testSimpleASRBWithExtraSpace() {
		...
		Assert.assertEquals("Must be an ASRB instruction", "ASRB", aslInstruction.instruction)
	}
	 ...
	 */
	@Test 
	def void testASRBWithDuplicateLabel() {
		val result = parseHelper.parse('''
		; -----------------------------------------
				   	ORG    			$8000
		Const	   	EQU          	5
	...
	def void testASRBAssembly() {
		...
		val line = engine.getAssembledLine(2) as AssembledASRBInstruction
		Assert.assertEquals("Check opcode length", 1, line.opcode.length)
		Assert.assertEquals("Check opcode", 0x57, line.opcode.get(0))
		Assert.assertEquals("Check operand length", 0, line.operand.length)
		Assert.assertEquals("Check label", "Start" , line.label)
		Assert.assertEquals("Check comment", "; 57   ASRB" , line.comment)
	}
}
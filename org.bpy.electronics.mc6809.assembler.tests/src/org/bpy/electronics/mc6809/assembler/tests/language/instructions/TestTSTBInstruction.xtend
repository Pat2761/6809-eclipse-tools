...
class TestTSTBInstruction {
	 ...
	 */
	@Test 
	def void testTSTBWithDuplicateLabel() {
		val result = parseHelper.parse('''
		; -----------------------------------------
				   	ORG    			$8000
		Const	   	EQU          	5
	...
	def void testTSTBAssembly() {
		...
		val line = engine.getAssembledLine(2) as AssembledTSTBInstruction
		Assert.assertEquals("Check opcode length", 1, line.opcode.length)
		Assert.assertEquals("Check opcode", 0x5D, line.opcode.get(0))
		Assert.assertEquals("Check operand length", 0, line.operand.length)
		Assert.assertEquals("Check label", "Start" , line.label)
		Assert.assertEquals("Check comment", "; 5D   TSTB" , line.comment)
	}
}
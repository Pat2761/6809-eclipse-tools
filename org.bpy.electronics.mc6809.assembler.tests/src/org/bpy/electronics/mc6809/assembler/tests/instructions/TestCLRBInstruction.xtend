...
class TestCLRBInstruction {
	...
	def void testSimpleCLRBWithExtraSpace() {
		...
		Assert.assertEquals("Must be an CLRB instruction", "CLRB", aslInstruction.instruction)
	}
	 ...
	 */
	@Test 
	def void testCLRBWithDuplicateLabel() {
		val result = parseHelper.parse('''
		; -----------------------------------------
				   	ORG    			$8000
		Const	   	EQU          	5
	...
	def void testCLRBAssembly() {
		...
		val line = engine.getAssembledLine(2) as AssembledCLRBInstruction
		Assert.assertEquals("Check opcode length", 1, line.opcode.length)
		Assert.assertEquals("Check opcode", 0x5F, line.opcode.get(0))
		Assert.assertEquals("Check operand length", 0, line.operand.length)
		Assert.assertEquals("Check label", "Start" , line.label)
		Assert.assertEquals("Check comment", "; 5F   CLRB" , line.comment)
	}
}
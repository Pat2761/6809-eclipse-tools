...
public class TestCLRBInstruction {
	...
	public void testSimpleCLRBWithExtraSpace() {
		...
		try {
		   ...
		   SourceLine line = result.getSourceLines().get(2);
			...
			Assert.assertTrue("Must be an CLRB directive line", instructionLine.getInstruction() instanceof ClrInstruction);
		} catch (Exception e) {
	...
	public void testSimpleCLRBWithExtraSpaceWithComment() {
		StringBuilder strBuilder = new StringBuilder();
		strBuilder.append("; -----------------------------------------\n");
		strBuilder.append("	       CLRB  			; It is a comment\n");
		strBuilder.append("	       CLRB\n"); 
		Model result;
		try {
			result = parseHelper.parse(strBuilder.toString());
	...
	/**
	 * Check Assembled CLRB
	 */
	@Test 
	public void testAssembledCLRB() {
		StringBuilder strBuilder = new StringBuilder();
		strBuilder.append("; -----------------------------------------\n");
		strBuilder.append("	      	ORG    $8000\n");
		strBuilder.append("LabelClrb	CLRB					; Clrb comment\n");
		try {
			Model result = parseHelper.parse(strBuilder.toString());
			...
			AssembledCLRBInstruction line = (AssembledCLRBInstruction)engine.getAssembledLine(2);
			Assert.assertEquals("Check label", "LabelClrb" , line.getLabel());
			Assert.assertEquals("Check comment", "; Clrb comment" , line.getComment());
			...
			Assert.assertEquals("Check Opcode code",0x5F,code[0]);

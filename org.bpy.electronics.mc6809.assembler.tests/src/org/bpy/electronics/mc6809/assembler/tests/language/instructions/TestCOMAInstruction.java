...
public class TestCOMAInstruction {
	...
	public void testSimpleCOMAWithExtraSpace() {
		...
		try {
		   ...
		   SourceLine line = result.getSourceLines().get(2);
			...
			Assert.assertTrue("Must be an COMA directive line", instructionLine.getInstruction() instanceof ComInstruction);
		} catch (Exception e) {
	...
	public void testSimpleCOMAWithExtraSpaceWithComment() {
		StringBuilder strBuilder = new StringBuilder();
		strBuilder.append("; -----------------------------------------\n");
		strBuilder.append("	       COMA  			; It is a comment\n");
		strBuilder.append("	       COMA\n"); 
		Model result;
		try {
			result = parseHelper.parse(strBuilder.toString());
	...
	/**
	 * Check Assembled COMA
	 */
	@Test 
	public void testAssembledCOMA() {
		StringBuilder strBuilder = new StringBuilder();
		strBuilder.append("; -----------------------------------------\n");
		strBuilder.append("	      	ORG    $8000\n");
		strBuilder.append("LabelComa	COMA					; Coma comment\n");
		try {
			Model result = parseHelper.parse(strBuilder.toString());
			...
			AssembledCOMAInstruction line = (AssembledCOMAInstruction)engine.getAssembledLine(2);
			Assert.assertEquals("Check label", "LabelComa" , line.getLabel());
			Assert.assertEquals("Check comment", "; Coma comment" , line.getComment());
			...
			Assert.assertEquals("Check Opcode code",0x43,code[0]);

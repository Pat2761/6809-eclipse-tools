...
/**
 * Allow to parse an LBEQ Instruction
 * 
 * @author briand
 *
 */
public class AssembledLBEQInstruction extends AbstractRelativeBranchInstruction {
	 ...
	 */
	public AssembledLBEQInstruction(AssemblerEngine engine) {
		super(engine);
	}
	...
	public void parsePass1(EObject instruction, int currentPcValue, int lineNumber) {
		this.instruction = (BeqInstruction) instruction;
	...
	public void parsePass2() {
		...
		if (instruction.getOperand().getOffset() != null) {
			...
			if (label != null) {
				...
				if (targetLine != null) {
					computeOperand(targetLine.getPcAddress(), WORD_MODE, AssemblerPackage.eINSTANCE.getBeqInstruction_Operand());
				} else {
					AssemblerErrorDescription problemDescription = new AssemblerErrorDescription("Label " + label + " isn't defined",
							AssemblerPackage.eINSTANCE.getBeqInstruction_Operand(), InstructionValidator.MISSING_LABEL);
	 ...
	 */
	public BeqInstruction getInstruction() {
		return instruction;
	}

	@Override
	public void setOpcode(AddressingMode mode) {
		opcodeBytes = new int[] { 0x10, 0x27 };
	}

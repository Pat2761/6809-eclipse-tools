...
/**
 * Allow to parse an LBRA Instruction
 * 
 * @author briand
 *
 */
public class AssembledLBRAInstruction extends AbstractRelativeBranchInstruction {

	/** Reference on the instruction */
	private BraInstruction instruction;
    ...
    *
	 ...
	 */
	public AssembledLBRAInstruction(AssemblerEngine engine) {
		super(engine);
	}
	...
	public void parsePass1(EObject instruction, int currentPcValue, int lineNumber) {
		this.instruction = (BraInstruction) instruction;
	...
	public void parsePass2() {
		...
		if (label != null) {
			...
			if (targetLine != null) {
				computeOperand(targetLine.getPcAddress(), WORD_MODE, AssemblerPackage.eINSTANCE.getBraInstruction_Operand());
			} else {
				AssemblerErrorDescription problemDescription = new AssemblerErrorDescription("Label " + label + " isn't defined",
						AssemblerPackage.eINSTANCE.getBraInstruction_Operand(), InstructionValidator.MISSING_LABEL);
	 ...
	 */
	public BraInstruction getInstruction() {
		return instruction;
	}

	@Override
	public void setOpcode(AddressingMode mode) {
		opcodeBytes =  new int[] {0x16};
	}

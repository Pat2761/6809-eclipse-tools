...
/**
 * Allow to parse an BGT Instruction
 * 
 * @author briand
 *
 */
public class AssembledBGTInstruction extends AbstractRelativeBranchInstruction {
	 ...
	 */
	public AssembledBGTInstruction(AssemblerEngine engine) {
		super(engine);
	}
	...
	public void parsePass1(EObject instruction, int currentPcValue, int lineNumber) {
		this.instruction = (BgtInstruction) instruction;
	...
	public void parsePass2() {
		...
		if (instruction.getOperand().getOffset() != null) {
			...
			if (label != null) {
				...
				if (targetLine != null) {
					computeOperand(targetLine.getPcAddress(), BYTE_MODE, AssemblerPackage.eINSTANCE.getBgtInstruction_Operand());
				} else {
					AssemblerErrorDescription problemDescription = new AssemblerErrorDescription("Label " + label + " isn't defined",
							AssemblerPackage.eINSTANCE.getBgtInstruction_Operand(), InstructionValidator.MISSING_LABEL);

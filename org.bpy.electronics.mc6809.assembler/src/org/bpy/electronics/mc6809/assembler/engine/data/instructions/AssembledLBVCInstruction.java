...
/**
 * Allow to parse an LBVC Instruction
 * 
 * @author briand
 *
 */
public class AssembledLBVCInstruction extends AbstractRelativeBranchInstruction {
    ...
    *
	 ...
	 */
	public AssembledLBVCInstruction(AssemblerEngine engine) {
		super(engine);
	}
	...
	public void parsePass1(EObject instruction, int currentPcValue, int lineNumber) {
		this.instruction = (BvcInstruction) instruction;
	...
	public void parsePass2() {
		...
		if (label != null) {
			...
			if (targetLine != null) {
				computeOperand(targetLine.getPcAddress(), WORD_MODE, AssemblerPackage.eINSTANCE.getBvcInstruction_Operand());
			} else {
				AssemblerErrorDescription problemDescription = new AssemblerErrorDescription("Label " + label + " isn't defined",
						AssemblerPackage.eINSTANCE.getBvcInstruction_Operand(), InstructionValidator.MISSING_LABEL);
	 ...
	 */
	public BvcInstruction getInstruction() {
		return instruction;
	}

	@Override
	public void setOpcode(AddressingMode mode) {
		opcodeBytes =  new int[] {0x10, 0x28};
	}

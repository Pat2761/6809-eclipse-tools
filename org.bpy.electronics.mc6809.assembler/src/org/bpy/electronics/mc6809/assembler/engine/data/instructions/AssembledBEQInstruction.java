...
/**
 * Allow to parse an BEQ Instruction
 * 
 * @author briand
 *
 */
public class AssembledBEQInstruction extends AbstractRelativeBranchInstruction {
	 ...
	 */
	public AssembledBEQInstruction(AssemblerEngine engine) {
		super(engine);
	}
	...
	public void parsePass2() {
		...
		if (instruction.getOperand().getOffset() != null) {
			...
			if (label != null) {
				...
				if (targetLine != null) {
					computeOperand(targetLine.getPcAddress(), BYTE_MODE, AssemblerPackage.eINSTANCE.getBeqInstruction_Operand());
				} else {

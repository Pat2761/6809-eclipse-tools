...
/**
 * Allow to parse an LBLO Instruction
 * 
 * @author briand
 *
 */
public class AssembledLBLOInstruction extends AbstractRelativeBranchInstruction {
    ...
    *
	 ...
	 */
	public AssembledLBLOInstruction(AssemblerEngine engine) {
		super(engine);
	}
	...
	public void parsePass1(EObject instruction, int currentPcValue, int lineNumber) {
		this.instruction = (BloInstruction) instruction;
	...
	public void parsePass2() {
		...
		if (label != null) {
			...
			if (targetLine != null) {
				computeOperand(targetLine.getPcAddress(), WORD_MODE, AssemblerPackage.eINSTANCE.getBloInstruction_Operand());
			} else {
				AssemblerErrorDescription problemDescription = new AssemblerErrorDescription("Label " + label + " isn't defined",
						AssemblerPackage.eINSTANCE.getBloInstruction_Operand(), InstructionValidator.MISSING_LABEL);
	 ...
	 */
	public BloInstruction getInstruction() {
		return instruction;
	}

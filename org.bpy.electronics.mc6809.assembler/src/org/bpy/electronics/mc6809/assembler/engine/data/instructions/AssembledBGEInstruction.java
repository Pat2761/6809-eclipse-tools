...
/**
 * Allow to parse an BGE Instruction
 * 
 * @author briand
 *
 */
public class AssembledBGEInstruction extends AbstractRelativeBranchInstruction {
    ...
    *
	 ...
	 */
	public AssembledBGEInstruction(AssemblerEngine engine) {
		super(engine);
	}
	...
	public void parsePass1(EObject instruction, int currentPcValue, int lineNumber) {
		this.instruction = (BgeInstruction) instruction;
	...
	public void parsePass2() {
		...
		if (label != null) {
			...
			if (targetLine != null) {
				computeOperand(targetLine.getPcAddress(), BYTE_MODE, AssemblerPackage.eINSTANCE.getBgeInstruction_Operand());
			} else {
	 ...
	 */
	public BgeInstruction getInstruction() {
		return instruction;
	}

	@Override
	public void setOpcode(AddressingMode mode) {
		opcodeBytes =  new int[] {0x2C};
	}

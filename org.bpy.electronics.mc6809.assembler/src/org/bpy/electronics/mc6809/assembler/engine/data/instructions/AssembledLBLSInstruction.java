...
/**
 * Allow to parse an LBLS Instruction
 * 
 * @author briand
 *
 */
public class AssembledLBLSInstruction extends AbstractRelativeBranchInstruction {
    ...
    *
	 ...
	 */
	public AssembledLBLSInstruction(AssemblerEngine engine) {
		super(engine);
	}
	...
	public void parsePass1(EObject instruction, int currentPcValue, int lineNumber) {
		this.instruction = (BlsInstruction) instruction;
	...
	public void parsePass2() {
		...
		if (label != null) {
			...
			if (targetLine != null) {
				computeOperand(targetLine.getPcAddress(), WORD_MODE, AssemblerPackage.eINSTANCE.getBlsInstruction_Operand());
			} else {
	 ...
	 */
	public BlsInstruction getInstruction() {
		return instruction;
	}

	@Override
	public void setOpcode(AddressingMode mode) {
		opcodeBytes =  new int[] {0x10, 0x23};
	}

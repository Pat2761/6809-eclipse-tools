...
/**
 * Allow to parse an BRN Instruction
 * 
 * @author briand
 *
 */
public class AssembledBRNInstruction extends AbstractRelativeBranchInstruction {

	/** Reference on the instruction */
	private BrnInstruction instruction;
    ...
    *
	 ...
	 */
	public AssembledBRNInstruction(AssemblerEngine engine) {
		super(engine);
	}
	...
	public void parsePass1(EObject instruction, int currentPcValue, int lineNumber) {
		this.instruction = (BrnInstruction) instruction;
	...
	public void parsePass2() {
		...
		if (label != null) {
			...
			if (targetLine != null) {
				computeOperand(targetLine.getPcAddress(), BYTE_MODE, AssemblerPackage.eINSTANCE.getBrnInstruction_Operand());
			} else {
				AssemblerErrorDescription problemDescription = new AssemblerErrorDescription("Label " + label + " isn't defined",
						AssemblerPackage.eINSTANCE.getBrnInstruction_Operand(), InstructionValidator.MISSING_LABEL);
	 ...
	 */
	public BrnInstruction getInstruction() {
		return instruction;
	}

	@Override
	public void setOpcode(AddressingMode mode) {
		opcodeBytes =  new int[] {0x21};
	}

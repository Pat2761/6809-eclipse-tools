...
/**
 * Allow to parse an LBMI Instruction
 * 
 * @author briand
 *
 */
public class AssembledLBMIInstruction extends AbstractRelativeBranchInstruction {

	/** Reference on the instruction */
	private BmiInstruction instruction;
    ...
    *
	 ...
	 */
	public AssembledLBMIInstruction(AssemblerEngine engine) {
		super(engine);
	}
	...
	public void parsePass1(EObject instruction, int currentPcValue, int lineNumber) {
		this.instruction = (BmiInstruction) instruction;
	...
	public void parsePass2() {
		...
		if (label != null) {
			...
			if (targetLine != null) {
				computeOperand(targetLine.getPcAddress(), WORD_MODE, AssemblerPackage.eINSTANCE.getBmiInstruction_Operand());
			} else {
				AssemblerErrorDescription problemDescription = new AssemblerErrorDescription("Label " + label + " isn't defined",
						AssemblerPackage.eINSTANCE.getBmiInstruction_Operand(), InstructionValidator.MISSING_LABEL);
	 ...
	 */
	public BmiInstruction getInstruction() {
		return instruction;
	}

	@Override
	public void setOpcode(AddressingMode mode) {
		opcodeBytes =  new int[] {0x10, 0x2B};
	}

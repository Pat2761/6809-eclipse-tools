...
/**
 * Allow to parse an LBCS Instruction
 * 
 * @author briand
 *
 */
public class AssembledLBCSInstruction extends AbstractRelativeBranchInstruction {

	/** Reference on the instruction */
	private BcsInstruction instruction;
	 ...
	 */
	public AssembledLBCSInstruction(AssemblerEngine engine) {
		super(engine);
	}
	...
	public void parsePass1(EObject instruction, int currentPcValue, int lineNumber) {
		this.instruction = (BcsInstruction) instruction;
	...
	public void parsePass2() {
		...
		if (instruction.getOperand().getOffset() != null) {
			...
			if (label != null) {
				...
				if (targetLine != null) {
					computeOperand(targetLine.getPcAddress(), WORD_MODE, AssemblerPackage.eINSTANCE.getBcsInstruction_Operand());
				} else {
					AssemblerErrorDescription problemDescription = new AssemblerErrorDescription("Label " + label + " isn't defined",
							AssemblerPackage.eINSTANCE.getBcsInstruction_Operand(), InstructionValidator.MISSING_LABEL);
	 ...
	 */
	public BcsInstruction getInstruction() {
		return instruction;
	}

	@Override
	public void setOpcode(AddressingMode mode) {
		opcodeBytes = new int[] { 0x10, 0x25 };
	}

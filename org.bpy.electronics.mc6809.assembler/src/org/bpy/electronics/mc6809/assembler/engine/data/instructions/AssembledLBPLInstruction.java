...
/**
 * Allow to parse an LBPL Instruction
 * 
 * @author briand
 *
 */
public class AssembledLBPLInstruction extends AbstractRelativeBranchInstruction {

	/** Reference on the instruction */
	private BplInstruction instruction;
	 ...
	 */
	public AssembledLBPLInstruction(AssemblerEngine engine) {
		super(engine);
	}
	...
	public void parsePass1(EObject instruction, int currentPcValue, int lineNumber) {
		this.instruction = (BplInstruction) instruction;
	...
	public void parsePass2() {
		...
		if (instruction.getOperand().getOffset() != null) {
			...
			if (label != null) {
				...
				if (targetLine != null) {
					computeOperand(targetLine.getPcAddress(), WORD_MODE, AssemblerPackage.eINSTANCE.getBplInstruction_Operand());
				} else {
					AssemblerErrorDescription problemDescription = new AssemblerErrorDescription("Label " + label + " isn't defined",
							AssemblerPackage.eINSTANCE.getBplInstruction_Operand(), InstructionValidator.MISSING_LABEL);
	 ...
	 */
	public BplInstruction getInstruction() {
		return instruction;
	}

	@Override
	public void setOpcode(AddressingMode mode) {
		opcodeBytes = new int[] { 0x10, 0x2A };
	}

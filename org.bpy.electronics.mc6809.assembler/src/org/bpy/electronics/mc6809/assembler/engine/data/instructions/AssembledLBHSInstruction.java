...
/**
 * Allow to parse an LBHS Instruction
 * 
 * @author briand
 *
 */
public class AssembledLBHSInstruction extends AbstractRelativeBranchInstruction {
	 ...
	 */
	public AssembledLBHSInstruction(AssemblerEngine engine) {
		super(engine);
	}
	...
	public void parsePass1(EObject instruction, int currentPcValue, int lineNumber) {
		this.instruction = (BhsInstruction) instruction;
	...
	public void parsePass2() {
		...
		if (instruction.getOperand().getOffset() != null) {
			...
			if (label != null) {
				...
				if (targetLine != null) {
					computeOperand(targetLine.getPcAddress(), WORD_MODE, AssemblerPackage.eINSTANCE.getBhsInstruction_Operand());
				} else {
					AssemblerErrorDescription problemDescription = new AssemblerErrorDescription("Label " + label + " isn't defined",
							AssemblerPackage.eINSTANCE.getBhsInstruction_Operand(), InstructionValidator.MISSING_LABEL);

...
public class AssemblerFormatter2 extends AbstractFormatter2 {
  ...
  private void formatSpaceOnly(final InstructionLine instructionLine, @Extension final IFormattableDocument document) {
    ...
    EObject _eContainer = instructionLine.eContainer();
    final SourceLine sourceLine = ((SourceLine) _eContainer);
    EObject _eContainer_1 = sourceLine.eContainer();
    final int indexSourceline = ((Model) _eContainer_1).getSourceLines().indexOf(sourceLine);
    final int labelLength = this.length(instructionLine.getLabel());
    int _xifexpression = (int) 0;
    if ((labelLength == 0)) {
      _xifexpression = (this.instructionPosition - 1);
    } else {
      _xifexpression = ((this.instructionPosition - labelLength) - 2);
    }
    int nbSpacesNeeded = _xifexpression;
    if ((this.junitPreference && (labelLength == 0))) {
      nbSpacesNeeded--;
    }
  ...
  private void formatTabOnly(final InstructionLine instructionLine, @Extension final IFormattableDocument document) {
    ...
    if (((estimatedPosition % this.tabSize) == 0)) {
      ...
      _xifexpression = ((estimatedPosition / this.tabSize) * 
        this.tabSize);
    }
